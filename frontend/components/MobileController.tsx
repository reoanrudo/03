    ...
    <div className="flex-1 flex-col min-h-screen w-full bg-slate-950 select-none touch-none font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-slate-900 border-b border-white/5">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${connected ? "bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.6)]" : "bg-red-500 animate-pulse"}`}
          ></div>
        ...
        <div className="text-right">
          ...
          <div className="font-mono text-sm font-black text-orange-500 leading-none">
            {roomId}
          </div>
        </div>
        <button
          onClick={onExit}
          className="ml-4 p-2 px-4 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-bold border border-white/5"
        >
          EXIT
        </button>
      </div>
      ...
      <div className="flex-1 flex-col relative bg-[#0f172a]">
        ...
        <div className="h-12 flex items-center justify-center bg-black/40 border-b border-white/10">
          {stringNames.map((name, i) => (
            <div
              key={i}
              className="text-center font-black text-slate-600 text-xs italic w-16"
            >
              {name}
            </div>
          ))}
        ...
        <div className="flex-1 flex flex-col px-4 pb-4">
          ...
          {Array.from({ length: totalFrets }).map((_, fIdx) => (
            <div key={fIdx} className="flex items-center py-2 relative">
                ...
                {stringNames.map((_, sIdx) => (
                  <div key={sIdx} className="flex-1 flex flex-col relative">
                    ...
                    <div className="flex-1 flex flex-row gap-1 z-20 relative">
                      ...
                      >
                        <div
                          className={`w-10 h-10 rounded-full border-2 flex items-center justify-center text-[12px] font-black transition-all ${
                      ...
                      {Array.from({ length: totalFrets }).map((_, fretIdx) => (
                        ...
                        >
                          <div
                            className={`w-12 h-12 rounded-full border-2 flex items-center justify-center text-[14px] font-black transition-all active:bg-white/5 ${
...
const ChordButton: React.FC<ChordBtnProps> = ({
  label,
  pattern,
  current,
  onClick,
      ...
      {isActive && barreFrets.length > 0 && (
        <div className="absolute bottom-0 left-0 right-0 flex gap-px px-2 pb-1">
          {barreFrets.map((fret) => (
            <div
              key={fret}
              className="flex-1 h-1 bg-orange-300/60 rounded-b shadow-[0_2px_2px_rgba(251,146,60,0.3)]"
            />
          ))}
        </div>
      )}
